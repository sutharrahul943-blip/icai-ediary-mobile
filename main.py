...
# --------------------------------------------------------------------------
# Crash logging - installed FIRST, before anything else can fail silently.
# --------------------------------------------------------------------------
def _crash_log_paths():
    paths = [
        "/sdcard/Download/ediary_crash_log.txt",
        "/sdcard/ediary_crash_log.txt",
        os.path.join(os.path.expanduser("~"), "ediary_crash_log.txt"),
    ]
    try:
        from kivy.app import App as _App
        app = _App.get_running_app()
        if app is not None:
            paths.insert(0, os.path.join(app.user_data_dir, "ediary_crash_log.txt"))
    except Exception:
        pass
    return paths


def write_crash_log(exc_text: str):
    written_to = None
    ...
    print("CRASH CAUGHT")
    if written_to:
        print(f"Crash log written to: {written_to}")
    else:
        print("Could not write crash log to any candidate path.")
    print(exc_text)
    print("=" * 60)
    return written_to
...
# --------------------------------------------------------------------------
# Everything below (including imports) is wrapped, since a crash during
# import (missing module, bad native lib, etc.) happens before any
# try/except inside a function would ever run.
# --------------------------------------------------------------------------
def _main():
